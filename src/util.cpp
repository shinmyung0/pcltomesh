...
int Util::getRandomNumber(const int _min, const int _max)
{
	srand(rand());
	srand(rand());
...
void Util::removeNANs(pcl::PointCloud<pcl::PointXYZ>::Ptr &_cloud)
{
	std::vector<int> mapping;
	removeNaNFromPointCloud(*_cloud, *_cloud, mapping);
}

pcl::PointCloud<pcl::Normal>::Ptr Util::getNormals(const pcl::PointCloud<pcl::PointXYZ>::Ptr &_cloud, const double _searchRadius)
{
...
bool Util::getCloudAndNormals(const std::string &_inputFile, pcl::PointCloud<pcl::PointNormal>::Ptr &_cloud, const double _estimationRadius)
{
	bool status = false;
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZ>(_inputFile, *dataXYZ) == 0)
	{
		...
		Util::removeNANs(dataXYZ);

		pcl::PointCloud<pcl::Normal>::Ptr normals = Util::getNormals(dataXYZ, _estimationRadius);
...
bool Util::isOriented(const Eigen::Vector3f &_normal, const Eigen::Vector3f &_normal0, const Eigen::Vector3f &_normal1, const Eigen::Vector3f &_normal2)
{
	int count = 0;
...
void Util::fixNormals(pcl::PointCloud<pcl::PointNormal>::Ptr &_cloud)
{
...
pcl::PointNormal Util::makePointNormal(const float _x, const float _y, const float _z, const float _nx, const float _ny, const float _nz, const float _curvature)
{
...
pcl::PointNormal Util::makePointNormal(const Eigen::Vector3f &_data)
{
	pcl::PointNormal point;
	point.x = _data.x();
	point.y = _data.y();
...
pcl::PointCloud<pcl::PointNormal>::Ptr Util::generateSurroundingSet(const pcl::PointNormal &_min, const pcl::PointNormal &_max)
{
